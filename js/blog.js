...
async function loadMarkdown(filename, converter) {  
  ...
  const maxHeight = 80; // 设置每篇日记的最大高度（以像素为单位）

  if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent)) {
    return `<h2>${title}<span class="subtitle"><br>date：${date}</span></h2><div class="contentback">${htmlContent}</div>`;//移动样式
  }else{
    return `<h2>${title}<span class="subtitle"> date：${date}</span></h2><div class="markdown-body" style="max-height: ${maxHeight}px; overflow: hidden;">${htmlContent}</div>`;//设置最大高度
  }
}
